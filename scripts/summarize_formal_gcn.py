#!/usr/bin/env python
"""Validate and aggregate all inductive-GCN result shards."""
...
def read_rows(path: Path) -> list[dict[str, str]]:
    if not path.is_file():
        raise FileNotFoundError(f"GCN result file is missing: {path}")
...
def main() -> int:
    ...
    for config in configs:
        source = paths["results_root"] / Path(config["output"]["directory"])
        rows.extend(read_rows(source / "results.csv"))
        manifest_path = source / "run_manifest.json"
        if not manifest_path.is_file():
            raise FileNotFoundError(f"GCN manifest is missing: {manifest_path}")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        if manifest.get("status") != "passed":
            raise RuntimeError(f"GCN shard did not pass: {manifest_path}")
        experiment_ids.append(str(manifest.get("experiment_id")))
    ...
    destination = paths["results_root"] / "derived" / "gcn_v2"
    if destination.exists():
        raise FileExistsError(f"Immutable derived directory already exists: {destination}")
    temporary = destination.parent / ".gcn_v2.inprogress"
    if temporary.exists():
    ...
    print("Inductive GCN summary status: passed")
