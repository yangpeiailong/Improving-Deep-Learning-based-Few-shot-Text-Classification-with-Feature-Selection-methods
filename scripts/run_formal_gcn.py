#!/usr/bin/env python
"""Preflight and run four immutable inductive-GCN result shards."""
...
def main() -> int:
    ...
    statuses: list[tuple[int, str, int, int, dict]] = []
    ...
    if args.preflight_only:
        print("Inductive GCN preflight status: passed")
        return 0

    runner = project_root / "scripts" / "run_gcn.py"
    for number, state, _, _, config in statuses:
        if state == "completed":
            print(f"Skipping completed shard {number}")
            continue
        with tempfile.TemporaryDirectory(prefix=f"fs_lrtc_gcn_{number}_") as temp:
            config_path = Path(temp) / f"gcn_shard{number}.json"
    ...
    print("All selected inductive GCN shards completed")
    return 0
