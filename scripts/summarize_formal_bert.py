#!/usr/bin/env python
"""Validate and aggregate all 16-dataset offline-BERT result shards."""
...
def read_rows(path: Path) -> list[dict[str, str]]:
    if not path.is_file():
        raise FileNotFoundError(f"BERT result file is missing: {path}")
...
def main() -> int:
    ...
    for config in configs:
        ...
        if not manifest_path.is_file():
            raise FileNotFoundError(f"BERT manifest is missing: {manifest_path}")
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        if manifest.get("status") != "passed":
            raise RuntimeError(f"BERT shard did not pass: {manifest_path}")
    ...
    destination = paths["results_root"] / "derived" / "bert_v3_full16"
    if destination.exists():
        raise FileExistsError(f"Immutable derived directory already exists: {destination}")
    temporary = destination.parent / ".bert_v3_full16.inprogress"
    if temporary.exists():
    ...
    print("Offline BERT summary status: passed")
