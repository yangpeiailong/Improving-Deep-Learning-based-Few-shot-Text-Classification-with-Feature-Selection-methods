...
if __name__ == '__main__':
    dataname, num_of_sample = sys.argv[1], sys.argv[2]
    # 加载数据集
    # dataname = '20newsgroup'
    # num_of_sample = str(1000)
    sample_data_file = 'e:/pythonwork/newclassification/dataset_after_preprocessing/' + dataname + '_' + \
                       num_of_sample + '.txt'
    label_data_file = 'e:/pythonwork/newclassification/dataset_after_preprocessing/' + dataname + '_' + num_of_sample\
                      + '_label.txt'
    ...
    with open('./GCN/middle_data/' + dataname+ '_' + \
                       num_of_sample + '.txt', 'w', encoding='utf-8') as f:
        for i in range(len(samples)):
            f.write(" ".join(samples[i]) + '\n')
    with open('./GCN/middle_data/' + dataname + '_' + \
              num_of_sample + '_label.txt', 'w', encoding='utf-8') as f:
    ...
    f = open('./GCN/graph/'+ dataname + '_' + \
                       num_of_sample + 'text_graph.pkl', 'wb')
    pickle.dump(G, f)
