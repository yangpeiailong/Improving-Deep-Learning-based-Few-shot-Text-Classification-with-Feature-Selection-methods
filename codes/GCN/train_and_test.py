...
if __name__ == '__main__':
    epoches, dataname, num_of_sample, op, para = int(sys.argv[1]), sys.argv[2], sys.argv[3], sys.argv[4], float(sys.argv[5])
    # 加载数据集
    # dataname = '20newsgroup'
    # num_of_sample = str(1000)
    sample_data_file = './GCN/middle_data/' + dataname + '_' + \
                       num_of_sample + '.txt'
    label_data_file = './GCN/middle_data/' + dataname + '_' + num_of_sample\
                      + '_label.txt'
    ...
    f = open('./GCN/graph/'+ dataname + '_' + \
                       num_of_sample + 'text_graph.pkl', 'rb')
    G = pickle.load(f)
    ...
    for i in range(k):
            ...
            if e % 50 == 0:
                ...
                if acc > best_val_acc:
                    torch.save(net.state_dict(),
                               'GCN/model/model_params_best_{data}_{samplenum}_GCN_{k_num}.pkl'.format(
                                   data=dataname, samplenum=num_of_sample, k_num=i))
        ...
        net.load_state_dict(torch.load(
            'GCN/model/model_params_best_{data}_{samplenum}_GCN_{k_num}.pkl'.format(data=dataname,
                                                                                        samplenum=num_of_sample,
                                                                                        k_num=i)))
    ...
    book.save('e:/pythonwork/newclassification/results/' + dataname + '_' + num_of_sample + '_GCN_criteria.xls')